...
public class HandTypeArrayListTest {
	...
	public void testHTAL() throws IOException {
		final HugeArrayBuilder<HandTypes> hab = new HugeArrayBuilder<HandTypes>() {
			{
				allocationSize = 4096;
			}
		};
		final HandTypesArrayList htal = new HandTypesArrayList(hab);
		htal.setSize(5);
		for (int i = 0, htalSize = htal.size(); i < htalSize; i++) {
			final HandTypes handTypes = htal.get(i);
